...
- [x] A ⚠ **Engine outdated** chip is in the title bar.
- [x] Click it. The dialog lists `godot/main.gd` as **unstamped** and the other
      two as **current**, and explains that runtime code is copied in at
      generation time.
- [x] **Update runtime.** It should report "updated 1 file", flip everything to
...
- [x] **▶ Play game** and look at an actor's feet. They should sit on the
...
- [X] An amber banner says *5 states reach the cell edge* and explains what that
      means.
- [x] The **stage** on the left is playing the animation.
- [x] The **filmstrip** on the right highlights the frame currently on stage.
- [x] Transport: `‹` / `❚❚` / `›` work; stepping pauses; the readout shows
      `frame N/M · 120ms`.
- [x] The **overlays** chip turns on the pixel grid and the green content box.
- [x] Click through the state chips. **`fall`** is the interesting one: frame 1
...
- [x] **Position in frame** → press ↓ a few times. The art moves down, the chip
      gains a `nudged` marker, and it saves.
- [x] **Reset to generated** puts it back and greys itself out.
- [x] Change **Loop** or **Every frame** and confirm the stage's playback changes.
